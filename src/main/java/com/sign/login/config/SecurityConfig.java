package com.sign.login.config;


import com.sign.login.service.MemberService;
...
@Configuration
@EnableWebSecurity //->@Configuration 클래스에 @EnableWebSecurity 어노테이션을 추가하여 Spring Security 설정할 클래스라 정의
@AllArgsConstructor
//->설정은  WebSebSecurityConfigurerAdapter 클래스를 상속받ㅇ아 메서드를 구현하는 것이 일반적 방법
// 인스턴스를 편리하게 생성하기 위한 클래스입니다.
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    //configure() 메서드를 오버라이딩하여, Security 설정을 잡아줌
    @Override
...
/*    antMatchers() 메서드로 특정 경로를 지정하며, permitAll(), hasRole() 메서드로 역할(Role)에 따른 접근 설정을 잡아줍니다.
    여기서 롤은 권한을 의미합니다. 즉 어떤 페이지는 관리지만 접근해야 하고, 어떤 페이지는 회원만 접근해야할 때 그 권한을 부여하기 위해 역할을 설정하는 것입니다.
    예를 들어,.antMatchers("/admin/**").hasRole("ADMIN")
    /admin 으로 시작하는 경로는 ADMIN 롤을 가진 사용자만 접근 가능합니다.antMatchers("/user/myinfo").hasRole("MEMBER")
    ...
    // 페이지 권한 설정
        ...
        .and()
            //403 예외처리
            .exceptionHandling().accessDeniedPage("/user/denide");
